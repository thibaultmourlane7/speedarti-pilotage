...
using (private.is_admin() or owner_member_id = private.current_team_member_id());
...
using (
  ...
  or exists (
    ...
    where cs.id = calendar_events.calendar_source_id
      and (cs.shared_with_team = true or i.owner_member_id = private.current_team_member_id())
  )
);
